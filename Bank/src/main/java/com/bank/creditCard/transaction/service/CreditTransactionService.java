...
public class CreditTransactionService {
    ...
    public CreditTransactionBean refundTransaction(Integer transactionId) {
        ...
        // 先存退款，再重算可用額度
        CreditTransactionBean savedRefund = creditTransactionRepository.save(refund);

        // ✅ 退款後同樣重算可用額度（currentBalance）
        recalcAndSaveCurrentBalance(original.getCardDetail().getCardId());
    ...
    /** 依所有交易「淨額」重算並保存可用額度(currentBalance) */
    private void recalcAndSaveCurrentBalance(Integer cardId) {
        CardDetailBean card = cardDetailRepository.findById(cardId)
                .orElseThrow(() -> new RuntimeException("卡片不存在，ID=" + cardId));

        BigDecimal limit = Optional.ofNullable(card.getCreditLimit()).orElse(BigDecimal.ZERO);

        // 已用 = 所有交易淨額（含退款為負數）
        BigDecimal used = creditTransactionRepository.findByCardDetail_CardId(cardId)
                .stream()
                .map(CreditTransactionBean::getAmount)
                .reduce(BigDecimal.ZERO, BigDecimal::add);

        // 可用 = limit - used，再夾在 0..limit
        BigDecimal available = limit.subtract(used);
        if (available.compareTo(BigDecimal.ZERO) < 0) available = BigDecimal.ZERO;
        if (available.compareTo(limit) > 0)           available = limit;

        card.setCurrentBalance(available);
